...
function buildInstructions(): string {
  const lines = [
    `EPA environmental data server covering facility compliance (ECHO), toxic releases (TRI), Superfund sites, drinking water systems (SDWIS), environmental-justice screening (EJScreen)${airNowEnabled ? ', and real-time air quality (AirNow)' : ''}.`,
    '- All tools are read-only. Most APIs are US federal government public domain data.',
    ...
    '- epa_get_ejscreen serves EJScreen v2.2 (2022) via the community-maintained EJAM API (Public Environmental Data Partners), which rehosts EJScreen after EPA discontinued public access in 2025 — not a live EPA endpoint; US coverage only.',
  ];
  if (airNowEnabled) {
    lines.push(
  ...
  lines.push(
    '- Typical workflows: epa_search_facilities → epa_get_facility → epa_search_violations (compliance audit); epa_search_superfund (proximity); epa_search_water_systems + has_violation=true (drinking water safety); epa_get_ejscreen (environmental-justice indicators for a point + buffer).',
  );
  return lines.join('\n');
}
