...
Thanks for using `epa-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.
...
`epa-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:

- **This repo** — a tool returns wrong EPA data, an upstream ECHO, DMAP, EJAM, or AirNow call fails, a schema doesn't match reality, or a description misleads the model.
